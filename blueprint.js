...
module.exports = {
	...
	Init: function() {
		...
		blueprint.set = function(rmName, limit) {
			let _BP = require("blueprint");
			_BP.setRoom(rmName, limit);
			return true;
		};
		...
		_.each(room.find(FIND_STRUCTURES).concat(room.find(FIND_CONSTRUCTION_SITES)), s => {
			...
			if (type == "road" || type == "container"
				...
				if (_.filter(flags, f => { return f.color == key["color"] && f.secondaryColor == key["secondaryColor"]; }).length > 0) {
					...
					let result = s.pos.createFlag(`blueprint_${s.pos.roomName}-${s.pos.x}-${s.pos.y}-${s.structureType}`, key["color"], key["secondaryColor"]);
