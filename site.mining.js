    ...
    run: function(rmColony, rmHarvest, popBurrower, popCarrier, popMiner, popReserver, popExtractor) {
        ...
        if (Object.keys(Game.rooms).includes(rmHarvest) && Game.rooms[rmHarvest].find(FIND_HOSTILE_CREEPS, 
            ...
            if (lDefender.length == 0) {
                utilColony.Spawn(rmColony, utilCreep.getBody_Soldier(utilColony.getRoom_Level(Game.rooms[rmColony])), null, {role: 'defender', room: rmHarvest});
            }
        }
        else if (lMiner.length < popMiner) {
            if (lMiner.length == 0) // Possibly colony wiped? Need restart?
                utilColony.Spawn(rmColony, [WORK, CARRY, CARRY, MOVE, MOVE], null, {role: 'miner', room: rmHarvest});
            else {
                utilColony.Spawn(rmColony, utilCreep.getBody_Worker(utilColony.getRoom_Level(Game.rooms[rmColony])), null, {role: 'miner', room: rmHarvest});
            }    
        }
            ...
            if (lCarrier.length == 0 && popCarrier > 0 && lMiner.length == 0) // Possibly colony wiped? Need restart?
                ...
                utilColony.Spawn(rmColony, utilCreep.getBody_Burrower(utilColony.getRoom_Level(Game.rooms[rmColony])), null, {role: 'burrower', room: rmHarvest});
            }
        }
        else if (lCarrier.length < popCarrier) {
            utilColony.Spawn(rmColony, utilCreep.getBody_Carrier(utilColony.getRoom_Level(Game.rooms[rmColony])), null, {role: 'carrier', room: rmHarvest});
        }
        else if (lReserver.length < popReserver) {
            var body = utilCreep.getBody_Reserver(utilColony.getRoom_Level(Game.rooms[rmColony]));
            if (body != null) {
                utilColony.Spawn(rmColony, body, null, {role: 'reserver', room: rmHarvest});
            }
        }
        else if (lExtractor.length < popExtractor) {
            utilColony.Spawn(rmColony, utilCreep.getBody_Worker(utilColony.getRoom_Level(Game.rooms[rmColony])), null, {role: 'extractor', room: rmHarvest});    
        }
